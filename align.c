...
#include <limits.h>
#include <sys/time.h>
#include <inttypes.h> // Include per PRIu64
...
int main(int argc, char *argv[]) {
	...
	unsigned long seq_length = atol( argv[1] );	//Lunghezza della sequenza DNA
	...
	prob_C += prob_G;	
	prob_A += prob_C;	
	//pat_samp = sample  || pat_rng = casuale
	int pat_rng_num = atoi( argv[5] );							//Numero di pattern casuali
	unsigned long pat_rng_length_mean = atol( argv[6] );		//Lunghezza media dei pattern casuali
	unsigned long pat_rng_length_dev = atol( argv[7] );			//Deviazione per i pattern casuali
	
	int pat_samp_num = atoi( argv[8] );							//Numero di pattern campionati dalla sequenza originale
	unsigned long pat_samp_length_mean = atol( argv[9] );		//Lunghezza media dei pattern campionati
	unsigned long pat_samp_length_dev = atol( argv[10] );		//Deviazione per i pattern campionati
	unsigned long pat_samp_loc_mean = atol( argv[11] );			//Posizione media in cui i pattern campionati dovrebbero iniziare nella sequenza principale
	unsigned long pat_samp_loc_dev = atol( argv[12] );			//Quanto possono variare le posizioni di inizio dei pattern rispetto alla posizione media
	...
	int pat_number = pat_rng_num + pat_samp_num;	// numero di pattern totali = pattern random + pattern sample
	unsigned long *pat_length = (unsigned long *)malloc( sizeof(unsigned long) * pat_number );	//array di variabili di tipo unsigned long usato per memorizzare le lunghezze dei pattern
	char **pattern = (char **)malloc( sizeof(char*) * pat_number );	//è un puntatore a un puntatore a carattere. In altre parole, pattern è una struttura che memorizza gli indirizzi delle stringhe (array di char) che saranno memorizzate nell'array
	...
	// Inizializzo un array di caratteri che mi servirà per rappresentare il tipo di pattern (random o sample) per tutti i pattern
	int ind;
	unsigned long lind;
	...
	switch( pat_samp_mix ) {
	//[A]fter: The sample patterns are sorted after the random ones in the patterns list
	case 'A':
		for( ind=0; ind<pat_rng_num; ind++ ) pat_type[ind] = PAT_TYPE_RNG;
		for( ; ind<pat_number; ind++ ) pat_type[ind] = PAT_TYPE_SAMP;
		break;
	//[B]efore: The sample patterns are sorted before the random ones in the patterns list
	case 'B':
		for( ind=0; ind<pat_samp_num; ind++ ) pat_type[ind] = PAT_TYPE_SAMP;
		for( ; ind<pat_number; ind++ ) pat_type[ind] = PAT_TYPE_RNG;
		break;
	//The sample and random patterns are interleaved
	default:
		//se non ho pattern casuali uso solo pattern sample
		if ( pat_rng_num == 0 ) {
			for( ind=0; ind<pat_number; ind++ ) pat_type[ind] = PAT_TYPE_SAMP;	
		}
		//se non ho pattern sample uso solo pattern random
		else if ( pat_samp_num == 0 ) {
			for( ind=0; ind<pat_number; ind++ ) pat_type[ind] = PAT_TYPE_RNG;	
		}
		//se ho più pattern sample
		else if ( pat_rng_num < pat_samp_num ) {
			int interval = pat_number / pat_rng_num;
			for( ind=0; ind<pat_number; ind++ ) 
				//cambia da sample a random ogni interval iterazioni in modo da produrre il numero atteso di pattern sample e random e alternandoli
				if ( (ind+1) % interval == 0 ) pat_type[ind] = PAT_TYPE_RNG;
				else pat_type[ind] = PAT_TYPE_SAMP;
		}
		//se ho più pattern random
		else {
			int interval = pat_number / pat_samp_num;
			for( ind=0; ind<pat_number; ind++ ) 
				//cambia da random a sample ogni interval iterazioni in modo da produrre il numero atteso di pattern sample e random e alternandoli
	...
	for( ind=0; ind<pat_number; ind++ ) {
		if ( pat_type[ind] == PAT_TYPE_RNG ) {
			//Alloca memoria per il pattern casuale specificando una lunghezza media (pat_rng_length_mean), una deviazione (pat_rng_length_dev), e la lunghezza massima (seq_length). 
			//Restituisce un puntatore al pattern generato e aggiorna la lunghezza effettiva in pat_length[ind]
			pattern[ind] = pattern_allocate( &random, pat_rng_length_mean, pat_rng_length_dev, seq_length, &pat_length[ind] );
			//Genera la sequenza di caratteri del pattern casuale basandosi sulle probabilità dei nucleotidi (G, C, A) e la lunghezza pat_length[ind]
	...
	/* 2.3.1. Other results related to patterns */ 		//inizializzo l'array che salva i found start
	unsigned long *pat_found;	//array di unsigned long 
	pat_found = (unsigned long *)malloc( sizeof(unsigned long) * pat_number );
	if ( pat_found == NULL ) {
		fprintf(stderr,"\n-- Error allocating aux pattern structure for size: %d\n", pat_number );
		exit( EXIT_FAILURE );
	}
	/* 2.3.2. Other results related to the main sequence */		//inizializzo l'array che salva i matches
	int *seq_matches;
	seq_matches = (int *)malloc( sizeof(int) * seq_length );	//ogni ingresso dell'array corrisponde al numero di match che partono dal corrispondente char della sequenza
	if ( seq_matches == NULL ) {
 ...
 */
	...
	for( ind=0; ind<pat_number; ind++) {
		pat_found[ind] = (unsigned long)NOT_FOUND;	//inizializzo l'array found_start con tutti not found 
	}
	for( lind=0; lind<seq_length; lind++) {			//inizializzo l'array matches con tutti not found 
	...
	for( pat=0; pat < pat_number; pat++ ) {		
		...
		if ( pat_found[pat] != (unsigned long)NOT_FOUND ) {
			/* 4.2.1. Increment the number of pattern matches on the sequence positions pat_found[pat]=start */
